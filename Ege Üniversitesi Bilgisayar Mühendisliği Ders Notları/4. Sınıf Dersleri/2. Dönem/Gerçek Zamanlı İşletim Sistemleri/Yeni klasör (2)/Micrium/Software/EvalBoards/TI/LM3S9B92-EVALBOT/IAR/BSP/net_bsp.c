...
static  void        NetDev_CfgClk_Stellaris            (NET_IF   *pif,
                                              NET_ERR  *perr);

static  void        NetDev_CfgIntCtrl_Stellaris        (NET_IF   *pif,
                                              NET_ERR  *perr);

static  void        NetDev_CfgGPIO_Stellaris           (NET_IF   *pif,
                                              NET_ERR  *perr);

static  CPU_INT32U  NetDev_ClkFreqGet_Stellaris        (NET_IF   *pif,
                                              NET_ERR  *perr);
...
const  NET_DEV_BSP_ETHER  NetDev_BSP_LM3S9Bxx = {   NetDev_CfgClk_Stellaris,        /*   Cfg clk(s)                             */
                                                    NetDev_CfgIntCtrl_Stellaris,    /*   Cfg int ctrl(s)                        */
                                                    NetDev_CfgGPIO_Stellaris,       /*   Cfg GPIO                               */
                                                    NetDev_ClkFreqGet_Stellaris     /*   Get clk freq                           */
                                                };
...
*
* Note(s)     : (1) Texas Instruments Stellaris devices feature clock gating to all peripherals. Therefore,
					we must first enable the ethernet peripheral before any other ethernet operations.
					Failure to do so will result in a device access fault.
					
					After being enabled the ethernet peripheral is reset as a precaution to return it to a known state.
                             ...
                             NET_ERR  *perr)
{
   ...
   (void)&pif;                                                  /* Prevent 'variable unused' compiler warning.          */
	...
	//
    // Enable and Reset the Ethernet Controller.  See Note (1)
    //
                                 ...
                                 NET_ERR  *perr)
{
	...
	//
	// Enable the Ethernet MAC's interrupt sources.
	//
	EthernetIntEnable(ETH_BASE, (ETH_INT_PHY | ETH_INT_MDIO |
