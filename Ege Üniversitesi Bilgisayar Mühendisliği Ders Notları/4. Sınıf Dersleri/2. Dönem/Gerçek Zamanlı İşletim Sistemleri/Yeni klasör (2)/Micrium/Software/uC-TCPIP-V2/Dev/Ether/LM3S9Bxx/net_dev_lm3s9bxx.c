...
#define FLASH_USERREG0                            0x400FE1E0    /* User Register 0 Used for storage.                    */
#define FLASH_USERREG1                            0x400FE1E4    /* User Register 1 MAC Address.                         */
#endif
                           ...
                           NET_ERR  *perr)
{
...
#if (NET_VERSION >= 21000u)                                     /* Validate Rx buf ix offset (see Note #2).             */
   ...
   (void)&buf_size_max;
...
#if (NET_VERSION >= 21000u)                                     /* Validate Tx buf ix offset (see Note #2).             */
    ...
    pdev_data = (NET_DEV_DATA *)pif->Dev_Data;
                                                                ...
                                                                /* ---------------------- INIT HW --------------------- */
#if (NET_VERSION >= 21000u)                                     /* Enable module clks (see Note #3).                    */
...
#if (NET_VERSION >= 21000u)                                     /* Configure Ethernet Controller GPIO (see Note #4).    */
    ...
    pdev_dma->DMA_ENA_CLR = DEF_BIT(NET_DEV_LM3S9Bxx_DMA_RX_CH)  /* Disable Rx & Tx channel                             */
...
#if (NET_VERSION >= 21000u)                                     /* Initialize ext int ctrl'r (see Note #5).             */
                            ...
                            NET_ERR  *perr)
{
    ...
    NetOS_Dev_CfgTxRdySignal(pif->Nbr,                          /* See Note #2.                                         */
                             1u,
                             perr);
    ...
    NetIF_AddrHW_SetHandler( pif->Nbr,                          /* See Note #4a.                                        */
                            &hw_addr[0],
                             sizeof(hw_addr),
                             perr);
    if (*perr == NET_IF_ERR_NONE) {                             /* Set dev MAC addr regs.                               */
        ...
        NetIF_AddrHW_GetHandler( pif->Nbr,
                                &hw_addr[0],
                                &hw_len,
                                 perr);
        ...
        if (valid == DEF_YES) {                                 /* Set dev MAC addr regs.                               */
            pdev->MACIA0 = ((((CPU_INT32U)hw_addr[0]) << (0 * DEF_INT_08_NBR_BITS)) |
                            (((CPU_INT32U)hw_addr[1]) << (1 * DEF_INT_08_NBR_BITS)) |
                            (((CPU_INT32U)hw_addr[2]) << (2 * DEF_INT_08_NBR_BITS)) |
                            (((CPU_INT32U)hw_addr[3]) << (3 * DEF_INT_08_NBR_BITS)));
			...
			*((CPU_REG32*) (&ulUser0)) = *((CPU_REG32*) FLASH_USERREG0);
			*((CPU_REG32*) (&ulUser1)) = *((CPU_REG32*) FLASH_USERREG1);
...
#else
    clk_freq = NetDev_MDC_ClkFreqGet(pif->Nbr, perr);
#endif
    if (*perr != NET_DEV_ERR_NONE) {
        return;
    }
                           ...
                           NET_ERR  *perr)
{
    ...
    if (pdev_data->TxBufCompPtr != (CPU_INT08U *)0) {               /* If NOT yet  tx'd, ...                            */
        NetOS_IF_TxDeallocTaskPost(pdev_data->TxBufCompPtr, &err);  /* ... dealloc tx buf (see Note #2a1).              */
   ...
   (void)&type;                                                 /* Prevent compiler warning.                            */
    ...
    if (DEF_BIT_IS_SET(status, LM3S9BXX_BIT_MACIM_TXER) == DEF_YES) {
        ...
        if (pdev_data->TxBufCompPtr != (CPU_INT08U *)0) {       /* Deallocate Tx buffer.                                */
            NetOS_IF_TxDeallocTaskPost(pdev_data->TxBufCompPtr, &err);
            pdev_data->TxBufCompPtr = (CPU_INT08U *)0;
        }

        NetOS_Dev_CfgTxRdySignal(pif->Nbr,                      /* Configure the ready signal.                          */
                                 1u,
                                 &err);
        ...
        if ((LM3S9BXX_DMA_CH_CTRL_GET_SIZE(pdma_ch_tbl->Ctrl) == 0u                                 ) &&
            (LM3S9BXX_DMA_CH_CTRL_GET_MODE(pdma_ch_tbl->Ctrl) == LM3S9BXX_BIT_DMACHCTL_XFERMODE_STOP)) {
             ...
             if (pdev_data->RxPktLen != 0u) {                   /* If a valid DMA packet transfer is completed          */
                ...
                NetOS_IF_RxTaskSignal(pif->Nbr, &err);          /* Signal Net IF RxQ Task for new rdy pkt.              */
             } else {
                 DEF_BIT_SET(pdev->MACIM, LM3S9BXX_BIT_MACIM_RX_ALL);
             }
        }
    ...
    if (DEF_BIT_IS_SET(status, LM3S9BXX_BIT_MACIS_TXEMP) == DEF_YES) {
         ...
         if ((LM3S9BXX_DMA_CH_CTRL_GET_SIZE(pdma_ch_tbl->Ctrl) == 0u                                 ) &&
            (LM3S9BXX_DMA_CH_CTRL_GET_MODE(pdma_ch_tbl->Ctrl) == LM3S9BXX_BIT_DMACHCTL_XFERMODE_STOP)) {
             ...
             pdev_data->TxBufCompPtr = (CPU_INT08U *)0;
             NetOS_IF_TxDeallocTaskPost(pdata, &err);           /* Deallocate the transmit buffer.                      */
