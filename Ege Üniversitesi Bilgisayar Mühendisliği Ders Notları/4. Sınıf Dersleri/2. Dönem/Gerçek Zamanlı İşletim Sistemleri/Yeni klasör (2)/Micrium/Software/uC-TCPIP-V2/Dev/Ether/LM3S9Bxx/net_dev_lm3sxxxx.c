...
#define FLASH_USERREG0          0x400FE1E0  // User Register 0 USED FOR STORAGE OF PRE-PROGRAMMED EVAL KIT
#define FLASH_USERREG1          0x400FE1E4  // User Register 1 MAC ADDRESSES

#endif //FLASH_USERREG0
                           ...
                           NET_ERR  *perr)
{
   ...
   (void)&buf_size_max;
    ...
    pdev_data = (NET_DEV_DATA *)pif->Dev_Data;
                                                                ...
                                                                /* ---------------------- INIT HW --------------------- */
#if (NET_VERSION >= 21000u)                                     /* Enable module clks (see Note #2).                    */
...
#if (NET_VERSION >= 21000u)                                     /* Configure Ethernet Controller GPIO (see Note #3).    */
...
#if (NET_VERSION >= 21000u)                                     /* Configure ext int ctrl'r (see Note #4).              */
...
*
*                                                               - RETURNED BY NetOS_Dev_CfgTxRdySignal() : -
                                  ...
                                  NET_DEV_ISR_TYPE   type)
{
   ...
   (void)&type;                                                 /* Prevent compiler warning.                            */
    ...
    if (DEF_BIT_IS_SET(status, LM3SXXXX_BIT_MACIS_RXINT) == DEF_YES) {
        pdev->MACIM &= ~(LM3SXXXX_BIT_MACIM_RXINT | LM3SXXXX_BIT_MACIM_FOV | LM3SXXXX_BIT_MACIM_RXER);
        NetOS_IF_RxTaskSignal(pif->Nbr, &err);                  /* Signal Net IF RxQ Task for each new rdy pkt.         */
    }
    ...
    if (DEF_BIT_IS_SET(status, LM3SXXXX_BIT_MACIS_TXEMP) == DEF_YES) {
         ...
         pdev_data->TxBufCompPtr = (CPU_INT08U *)0;
         NetOS_IF_TxDeallocTaskPost(pdata, &err);
